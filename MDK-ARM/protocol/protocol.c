  ...
  while(1)
  {
    ...
    switch (cmd_type)
    {
      ...
      case SET_P_I_D_CMD:
      {
        ...
        {
          set_p_i_d(&pid_roll3_structure, temp_p.f, temp_i.f, temp_d.f);    // 设置 P I D
        }
        else if (packet.ch == CURVES_CH2)
        {
          set_p_i_d(&pid_pitch3_structure, temp_p.f, temp_i.f, temp_d.f);    // 设置 P I D
        }
				else if (packet.ch == CURVES_CH3)
        {
          set_p_i_d(&pid_roll2_structure, temp_p.f, temp_i.f, temp_d.f);    // 设置 P I D
        }else if (packet.ch == CURVES_CH4)
        {
          set_p_i_d(&pid_pitch2_structure, temp_p.f, temp_i.f, temp_d.f);    // 设置 P I D
        }
//				else if (packet.ch == CURVES_CH5)
//        {
//          set_p_i_d(&pid_roll3_structure, temp_p.f, temp_i.f, temp_d.f);    // 设置 P I D
//        }
//				else if (packet.ch == CURVES_CH6)
//        {
...
//        }
      ...
      case SET_TARGET_CMD:
      {
        ...
        if (packet.ch == CURVES_CH1)    
        {
          set_pid_target(&pid_roll3_structure, target_temp);    // 设置目标值
        }
				else if (packet.ch == CURVES_CH2)
        {
          set_pid_target(&pid_pitch3_structure, target_temp);    // 设置 P I D
        }
//				else if (packet.ch == CURVES_CH3)
//        {
//          set_pid_target(&pid_roll2_structure, target_temp);    // 设置 P I D
//        }else if (packet.ch == CURVES_CH4)
//        {
//         set_pid_target(&pid_pitch2_structure, target_temp);   // 设置 P I D
//        }else if (packet.ch == CURVES_CH5)
//        {
//          set_pid_target(&pid_roll3_structure, target_temp);    // 设置 P I D
//        }
...
//      
      case STOP_CMD:
      {
				EventFlag=0x00;
        set_zero_all();              // 停止
				PID_iinit(&pid_roll3_structure, &pid_pitch3_structure);
...
void set_zero_all(void){
	uint8_t cmdtemp[8]={'A','0','0','B','0','0','0','0'};
	...
	cmdtemp[2]='9';
	HAL_UART_Transmit(&huart2,(uint8_t *)cmdtemp,8,1000);
	cmdtemp[1]='1';cmdtemp[2]='0';
	HAL_UART_Transmit(&huart2,(uint8_t *)cmdtemp,8,1000);
	cmdtemp[1]='1';cmdtemp[2]='1';
	HAL_UART_Transmit(&huart2,(uint8_t *)cmdtemp,8,1000);
	cmdtemp[1]='1';cmdtemp[2]='2';
	HAL_UART_Transmit(&huart2,(uint8_t *)cmdtemp,8,1000);
}
